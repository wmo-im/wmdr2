...
This directory contains the WMDR2 source schemas.  The schemas are managed
...
To generate the WMDR2 JSON schema, following these steps:
...
- click "Open"
- on the right hand pane, right-click the file `wmdrRecordGeoJSON.yaml` and select "Export"
- select the following from the dialogue box:
  - Format: JSON
  - References: Dereferened
  - X-Extensions: unchecked
- click "Save to file"
- save file to `schemas/wmdr2-bundled.json`
- commit changes
...
## Validating a WMDR2 record

```bash
# install the check-jsonschema Python package
pip3 install check-jsonschema

# validate a WMDR2 file on the command line
check-jsonschema --schemafile schemas/wmdr2-bundled.json examples/20250504_0-20008-0-NRB.json
```
