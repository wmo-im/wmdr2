...
Authors of observing facility metadata records published within the WIGOS Global Observing Facility Catalogue (GOFC) are required to comply with the WIGOS Metadata Representation (WMDR).  WMDR observing facility metadata shall therefore be compliant with OGC API - Records - Part 1: Core: Requirements Class: Record Core.

WMO shall publish guidance material to assist authors of WMDR observing facility metadata for maintaining
...
* "WIGOS Metadata Representation Core": This Requirements Class inherits from _OGC API — Records — Part 1: Core:
Requirements Class: Record Core_ which defines the requirements for a catalogue record.  The requirements
specified in the Requirements Class “Record Core" are mandatory for all implementations of WMDR.
